...
import { Wifi, Gamepad2, Search, Trash2, Filter, Archive, CheckSquare } from "lucide-react";
...
export function History() {
  const { sessions, archiveSessions, deleteSessions } = useApp();
  ...
  const stats = useMemo(() => {
    ...
    const totalDuration = completedSessions.reduce((sum, s) => sum + s.elapsedTime, 0);
    return {
      count: completedSessions.length,
      totalRevenue,
      avgRevenue: completedSessions.length > 0 ? totalRevenue / completedSessions.length : 0,
      totalDuration,
    };
  }, [completedSessions]);

  const allSelected = completedSessions.length > 0 && selectedIds.length === completedSessions.length;
  const someSelected = selectedIds.length > 0 && selectedIds.length < completedSessions.length;

  const toggleSelectAll = () => {
    if (allSelected) {
      setSelectedIds([]);
    } else {
      setSelectedIds(completedSessions.map((s) => s.id));
    }
  };
  ...
  const handleBulkAction = async () => {
    ...
    try {
      if (actionType === "archive") {
        await archiveSessions(selectedIds);
        toast.success(`${selectedIds.length} session(s) archivée(s)`);
    ...
    <div className="p-4 sm:p-6 lg:p-8">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>Sessions ({completedSessions.length})</CardTitle>
              {isAdmin && completedSessions.length > 0 && (
                <div className="flex items-center gap-2">
                  <Checkbox
                    checked={allSelected}
      ...
      <AlertDialog open={!!actionType} onOpenChange={() => setActionType(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {actionType === "archive" ? "Archiver les sessions ?" : "Supprimer les sessions ?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {actionType === "archive" ? (
                <>
                  {selectedIds.length} session(s) sera(ont) archivée(s). Les sessions archivées
                  resteront dans la base de données mais ne s'afficheront plus dans l'historique.
                  Vous pourrez les consulter dans la page Archives.
                </>
              ) : (
                <>
