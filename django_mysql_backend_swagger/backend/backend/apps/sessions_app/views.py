...
class SessionViewSet(viewsets.ModelViewSet):
    ...
    def pay(self, request, pk=None):
        ...
        if session.status == Session.Status.ARCHIVED:
            raise serializers.ValidationError({
                "status": "Session deja archivee"
            })

        if session.status != Session.Status.COMPLETED:
            raise serializers.ValidationError({
                "status": "Termine d'abord la session avant de confirmer le paiement"
            })
        ...
        session.save(update_fields=["payment_status", "paid_at", "paid_by", "updated_at"])

        SessionEvent.objects.create(
            session=session,
            event_type=SessionEvent.EventType.PAY,
            user=request.user,
            note=f"Paiement confirme: {session.final_price or 0} Ar",
        )
